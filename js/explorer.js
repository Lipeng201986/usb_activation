...
const usagePageName = usagePage => {
    const kUsagePageNames = {
        // 0x0000 is Undefined
...
// Returns a human-readable name for the HID usage identified by the 32-bit
// |usage| value. The usage value must include the usage page in the upper
// bytes.
//
// Usage IDs and names are sourced from the following documents:
//
// HID Usage Tables version 1.22
// https://usb.org/document-library/hid-usage-tables-122
//
// HID Point of Sale Usage Tables version 1.02
// https://www.usb.org/document-library/hid-point-sale-usage-tables-102
//
// Usage Tables for HID Power Devices version 1.1
// https://www.usb.org/document-library/usage-tables-hid-power-devices-11
//
// Device Class Definition for PID version 1.0
// https://www.usb.org/document-library/device-class-definition-pid-10
//
// Monitor Control Class version 1.0
// https://www.usb.org/document-library/monitor-control-class-10
//
// OOAD Arcade Usage Table version 1.100
// https://www.usb.org/document-library/oaad-arcade-usage-table
//
// Review Request 101: Emoji Picker
// https://www.usb.org/sites/default/files/hutrr101-emojipicker.pdf
const usageName = usage => {
    ...
    if (usagePage === 0x0009)
        return usageString + ' (' + usagePageName(usagePage) + ' Button ' + usageId + ')';
    if (usagePage === 0x000A)
        return usageString + ' (' + usagePageName(usagePage) + ' Instance ' + usageId + ')';
    if (usagePage === 0x0010)
        return usageString + ' (' + usagePageName(usagePage) + ' U+' + hex16(usageId) + ')';
    if (usagePage === 0x0081)
        return usageString + ' (' + usagePageName(usagePage) + ' ENUM_' + usageId + ')';

    // Include the usage name, if available.
    if (usage in kUsageNames)
        return usageString + ' (' + usagePageName(usagePage) + ' > ' + kUsageNames[usage] + ')';

    return usageString + ' (' + usagePageName(usagePage) + ' usage 0x' + hex16(usageId) + ')';
}
...
const usagesAsString = item => {
    if (item.isRange) {
        ...
        return 'Usages: ' +
            usageName(item.usageMinimum) + ' to ' +
            usageName(item.usageMaximum);
    }

    let usageStrings = [];
    for (const usage of item.usages)
        usageStrings.push(usageName(usage));
...
const addUnitFactor = (numerator, denominator, unitFactorName, unitFactorExponent) => {
...
// |type|.
const formatReportListInfo = (reports, type) => {
    let reportInfo = '';
...
// Updates the device info display for the currently selected device.
const updateDeviceInfo = () => {
    let textarea = document.getElementById('deviceInfo');
    if (selectedDevice == null) {
        textarea.innerHTML = '';
        return;
    }
    ...
    let deviceInfo =
        ...
        'vendorId:    0x' + hex16(selectedDevice.vendorId) + ' (' + selectedDevice.vendorId + ')\n' +
        'productId:   0x' + hex16(selectedDevice.productId) + ' (' + selectedDevice.productId + ')\n' +
        'opened:      ' + (selectedDevice.opened ? 'true' : 'false') + '\n';
    ...
    if (selectedDevice.collections.length == 0) {
        ...
        for (let i = 0; i < selectedDevice.collections.length; ++i) {
            ...
            deviceInfo += '  Usage ' + hex16(c.usagePage) + ':' + hex16(c.usage) + '\n';
